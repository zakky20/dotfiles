...
keys = [
    ...
    Key([mod], "d", lazy.spawn("bash -c '~/.config/rofi/launchers/type-4/launcher.sh'"), desc="App launcher"),
...
group_labels = ["󰈹 ", " ", " ", " ", " ", " ", " ", " ", " "]
# group_labels = ["WWW", "DEV", "CHAT", "MUS", "DOC", "SYS", "VID", "GFX", "VBOX", "MISC"]
...
def init_widgets_list():
    widgets_list = [
        ...
        widget.Memory(
                 ...
                 padding = 8, 
                 mouse_callbacks = {'Button1': lambda: qtile.cmd_spawn(terminal + ' -e htop')},
            format = '{MemUsed: .0f} {mm}B /{MemTotal: .0f} {mm}B',
                 fmt = ' {}',
                 ),
        widget.DF(
                 update_interval = 60,
...
def init_screens():
    return [Screen(top=bar.Bar(widgets=init_widgets_screen1(), margin=[8, 10, 0, 10], size=42)),
            Screen(top=bar.Bar(widgets=init_widgets_screen2(), margin=[8, 10, 0, 10], size=42)),
            Screen(top=bar.Bar(widgets=init_widgets_screen2(), margin=[8, 10, 0, 10], size=42))]
